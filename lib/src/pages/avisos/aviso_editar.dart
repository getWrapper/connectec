...
class _AvisoEditarPageState extends State<AvisoEditarPage>
  ...
  List<String> imgDel = new List<String>();
  List<String> imgExist = new List<String>();
  ...
  void animateButton(BuildContext context) async {
    ...
    if (connection) {
      ...
      if (actualizarAviso != null) {
          await avisoProvider.actualizarImgAviso(assets, avisoEditar.id, imgDel, imgExist);
  ...
  Widget buildGridView() {
    imgExist = new List();
    var imgNetwork = avisoEditar.getNoticiaImg();
    return GridView.count(
      crossAxisCount: 2,
      children: List.generate(images.length + assets.length, (index) {
        if(index < images.length){
          imgExist.add(images[index]);
return Container(
          ...
          child: Stack(
            ...
            children: <Widget>[
              FadeInImage(
          height: 300,
          image: NetworkImage(imgNetwork[index]),
          ...
          Positioned(top: 5, right: 5, child: GestureDetector(child: Icon(Icons.delete_forever, color: Theme.of(context).accentColor), onTap: (){
            ...
            imgExist.removeAt(index);
            imgDel.add(images[index]);
          ...
          return  Stack(
                      children: <Widget>[
                        AssetThumb(
              asset: assets[index-images.length],
              width: 300,
              height: 300,
            ),
            Positioned(top: 5, right: 5, child: GestureDetector(child: Icon(Icons.delete_forever, color: Theme.of(context).accentColor), onTap: (){
            setState(() {
            assets.removeAt(index-images.length);
            });
          },))
                      ],
          );
...
Widget _labelSelected(BuildContext context){
    Widget text = Container();
     if(assets.length == 1){
      text= Text('1 nuevo archivo seleccionado',style: Theme.of(context).textTheme.headline3 );
    }else{
      if(assets.length > 1){
      text= Text('${assets.length} nuevos archivos seleccionados', style: Theme.of(context).textTheme.headline3 );
    }
    }
